    ...
    <div className="w-full bg-white rounded-xl border border-[#DDE6ED] p-8 shadow-sm">
      ...
      <div className="flex justify-between items-start">
        <div className="flex gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-black leading-snug">{title}</h1>
      ...
      <div className="mt-6 space-y-10">
        ...
        {/* QUALIFICATION */}
        <section>
          <h3 className="font-bold text-lg flex items-center gap-2">🎓 Qualification</h3>
          <ul className="mt-3 text-gray-700">
            ...
            {!kualifikasi?.length && !kualifikasiTambahan?.length && <li>- Tidak ada kualifikasi</li>}
          </ul>
        </section>

        {/* RESPONSIBILITIES */}
        <section>
          <h3 className="font-bold text-lg flex items-center gap-2">📌 Responsibilities</h3>
          <ul className="mt-3 text-gray-700">
      ...
      {/* MODAL */}
      {isOpen && <ModalApply userId={userId} lowonganId={id} onClose={() => setIsOpen(false)} />}
    </div>
  );
};
