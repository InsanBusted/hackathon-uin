...
export default function TableProject({ data }: { data: any[] }) {
        ...
        {data.length === 0 ? (
          ...
          data.map((item) => (
            <TableRow key={item.id}>
              <TableCell>
                <Link href={`project/pelamar/${item.biodata.userId}`}>
                  {item?.biodata?.fullName ?? "-"}
                </Link>
              </TableCell>
              <TableCell>{item.lowongan.title}</TableCell>
